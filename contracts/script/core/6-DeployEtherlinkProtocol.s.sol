...
/**
 * @title DeployEtherlinkProtocol
 * @notice Deploy all protocol contracts and configure KiloLend on Etherlink (Chain ID 42793)
 * @dev Usage: 
 *   forge script script/core/6-DeployEtherlinkProtocol.s.sol --rpc-url $ETHERLINK_RPC_URL --broadcast
 */
contract DeployEtherlinkProtocol is Script {
    
    // Token addresses on Etherlink
    ...
    uint256 public constant XTZ_COLLATERAL_FACTOR = 0.75e18;  // 75% for native XTZ
    uint256 public constant USDT_COLLATERAL_FACTOR = 0.85e18; // 85% for stablecoin
    
    // Borrow caps (0 = no cap)
    uint256 public constant XTZ_BORROW_CAP = 0;    // No cap
    uint256 public constant USDT_BORROW_CAP = 0;  // No cap
    
    // Etherlink specific: 0.8 second block time = 39,420,000 blocks per year
    uint256 public constant BLOCKS_PER_YEAR = 39420000;
    ...
    function run() external {
        ...
        require(balance > 0.8 ether, "Insufficient balance for deployment (need at least 0.8 XTZ)");
    ...
    function _verifyPrerequisites() internal view {
        ...
        require(ORACLE != address(0), "Oracle address not set (set ETHERLINK_ORACLE_ADDRESS env var)");
    ...
    function _deployCTokens(address deployer) internal {
        ...
        // Deploy cXTZ for native XTZ token  
        cXTZ = new CEther(
            ComptrollerInterface(address(comptroller)),
    ...
    function _supportMarkets() internal {
        ...
        result = comptroller._supportMarket(CToken(address(cXTZ)));
        require(result == 0, "Failed to support cXTZ market");
        console.log("cXTZ market supported");
    }
    ...
    function _setRiskParameters() internal {
        ...
        console.log("cUSDT collateral factor:", USDT_COLLATERAL_FACTOR / 1e18);
        
        result = comptroller._setCollateralFactor(CToken(address(cXTZ)), XTZ_COLLATERAL_FACTOR);
        require(result == 0, "Failed to set cXTZ collateral factor");
        console.log("cXTZ collateral factor:", XTZ_COLLATERAL_FACTOR / 1e18);
    ...
    function _logDeploymentResults() internal view {
        ...
        console.log("cXTZ (Volatile - Native):", address(cXTZ));
        
        console.log("\nRisk Parameters:");
        console.log("USDT Collateral Factor:", USDT_COLLATERAL_FACTOR / 1e18);
        console.log("XTZ Collateral Factor:", XTZ_COLLATERAL_FACTOR / 1e18);
        console.log("Close Factor:", CLOSE_FACTOR / 1e18);
        console.log("Liquidation Incentive:", LIQUIDATION_INCENTIVE / 1e18);
        
        console.log("\nEtherlink Specific:");
        console.log("Block Time: 0.8 seconds");
        console.log("Blocks Per Year:", BLOCKS_PER_YEAR);
