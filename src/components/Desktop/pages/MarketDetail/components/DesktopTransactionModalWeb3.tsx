...
interface DesktopTransactionModalWeb3Props {
  isOpen: boolean;
  onClose: () => void;
  type: 'supply' | 'borrow';
  amount: string;
...
export const DesktopTransactionModalWeb3 = ({
  isOpen,
  onClose,
  type,
  amount,
  ...
  const checkRequirements = async () => {
    ...
    try {
      ...
      if (type === 'supply' && market.tokenAddress !== '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE') {
      ...
      if (type === 'supply' && market.marketAddress) {
        const isEntered = await isMarketEntered(account, market.marketAddress as string);
        ...
        <TransactionDetails>
          ...
          {type === 'borrow' && (
            <DetailRow>
              ...
              <DetailValue>{(amountNum * (market.borrowAPR / 100)).toFixed(4)} {displaySymbol}</DetailValue>
            </DetailRow>
          )}
          <DetailRow>
